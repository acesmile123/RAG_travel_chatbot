...
function App() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "bot",
      content:
        "Hello! I'm here to help with your travel plans. Where would you like to go?",
      sources: [],
    },
  ]);
  ...
  const faqs = [
    {
      id: 1,
      question: "Đây là trang web gì?",
      answer:
        "Hệ thống Trợ lý du lịch thông minh ứng dụng AI chuyên sâu cho 18 tỉnh thành Việt Nam.",
    },
    {
      id: 2,
      question: "Tôi có thể hỏi những gì ở đây?",
      answer:
        "Bạn có thể hỏi mọi thông tin về du lịch, ẩm thực và đi lại tại 18 tỉnh thành: Miền Bắc (Hà Nội, Hải Phòng, Quảng Ninh, Bắc Ninh, Cao Bằng, Lạng Sơn, Tuyên Quang) • Miền Trung (Đà Nẵng, Huế, Quảng Nam, Quảng Bình, Hà Tĩnh, Thanh Hóa) • Miền Nam (TP.HCM, Cần Thơ, Kiên Giang, Tây Ninh, Vũng Tàu). Chỉ cần nhập tên địa danh hoặc món ăn, AI sẽ hỗ trợ bạn ngay!",
    },
    {
      id: 3,
      question: "Chúng tôi là ai?",
      answer:
        "Nhóm sinh viên CS thực hiện đồ án về RAG & Fine-tuning LLM cho ngành du lịch Việt Nam.",
    },
    {
      id: 4,
      question: "Thông tin ở đây có chính xác không?",
      answer:
        "Có! Hệ thống dùng RAG để truy xuất dữ liệu từ sách báo và web uy tín, hạn chế lỗi ảo tưởng của AI.",
    },
    {
      id: 5,
      question: "Chatbot này có gì đặc biệt?",
      answer:
        "Văn phong gần gũi, sành sỏi như người bản địa nhờ quá trình Fine-tuning đặc thù.",
    },
    {
      id: 6,
      question: "Tôi có cần đăng nhập không?",
      answer:
        "Không cần. Bạn có thể sử dụng ngay lập tức với quyền riêng tư được đảm bảo tuyệt đối.",
    },
  ];
  ...
  const createNewSession = async () => {
    try {
      ...
      // Reset chat and set new session
      setMessages([
        {
          role: "bot",
          content:
  ...
  const removeSession = async (sessionId, event) => {
    ...
    try {
      ...
      // If viewing deleted session, reset
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const data = await sendMessageToAI(userMessage, currentSessionId, false);
      ...
      // Reload sessions to update the list with new message
      ...
      const errorMsg = {
        role: "bot",
        content:
          "Xin lỗi, đã có lỗi xảy ra. Vui lòng kiểm tra kết nối Backend và thử lại.",
        sources: [],
      };
    ...
    <div className="min-h-screen bg-[#fcfcfc] dark:bg-[#1a1a1a] flex flex-col">
      ...
      <header className="sticky top-0 z-10 border-b border-gray-200 dark:border-gray-700 py-6 bg-white dark:bg-gray-900 relative">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h1 className="text-2xl font-light tracking-tight text-gray-800 dark:text-gray-200">
            Travel Assistant
          </h1>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
            Your personal guide to exploring the world
          </p>
        </div>
      ...
      <div className="fixed left-1/2 bottom-10 z-50 w-full max-w-4xl px-6 -translate-x-1/2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${
              !input.trim() || isLoading
                ...
                : "bg-gray-800 dark:bg-gray-600 text-white hover:bg-gray-700 dark:hover:bg-gray-500 cursor-pointer"
            }`}
          >
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 text-center">
          Lưu ý: Mô hình có thể đưa ra câu trả lời không chính xác ở một số
          trường hợp, vì vậy hãy luôn kiểm chứng thông tin bạn nhé!
        </p>
      </div>

      {/* Sidebar overlay */}
      <div
        className={`fixed inset-0 z-50 ${sidebarOpen ? "block" : "pointer-events-none"}`}
          ...
          <nav className="p-4 space-y-2">
            ...
            <div>
              <button
                ...
                className="w-full text-left flex items-center justify-between text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded"
              >
                <span className="flex items-center gap-2">
                  Lịch sử hội thoại
                </span>
                <svg
                  className={`w-4 h-4 text-gray-500 dark:text-gray-400 transform transition-transform duration-200 ${historyOpen ? "rotate-180" : ""}`}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
              ...
              {/* Sessions list */}
              <div
                className={`${historyOpen ? "block" : "hidden"} mt-2 ml-3 space-y-1 text-sm text-gray-600 dark:text-gray-400`}
              >
                {isSyncing ? (
                  ...
                  sessions.map((session) => (
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => removeSession(session.session_id, e)}
            ...
            <div>
              ...
              >
                FAQs
                <svg
                  className={`w-4 h-4 text-gray-500 dark:text-gray-400 transform transition-transform duration-200 ${faqOpen ? "rotate-180" : ""}`}
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
              ...
              {/* FAQ list (toggleable) */}
              <div
                className={`${faqOpen ? "block" : "hidden"} mt-2 ml-3 space-y-1 text-sm text-gray-600 dark:text-gray-400`}
              >
                {faqs.map((faq) => (
                  <button
                    key={faq.id}
            ...
            <button
              ...
              className="w-full text-left text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded"
            >
              Báo lỗi / Góp ý
            </button>
          </nav>
      ...
      <div
        className={`fixed inset-0 z-50 ${selectedFaq ? "block" : "pointer-events-none"}`}
        aria-hidden={!selectedFaq}
      >
        <div
      ...
      <div
        className={`fixed inset-0 z-50 ${feedbackOpen ? "block" : "pointer-events-none"}`}
